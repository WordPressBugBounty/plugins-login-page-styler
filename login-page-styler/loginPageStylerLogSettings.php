...
function display_login_logs_table( $table_name, $title, $is_failed_login = false ) {
	...
	foreach ( $logs as $log ) {
		...
		if ( ! $is_failed_login ) {
			...
			// Display status (only for successful logins)
			...
			echo '<td>
            <a href="' . esc_url( admin_url( 'admin-post.php?action=lps_delete_login_log&log_id=' . esc_attr( $log->id ) ) ) . '" class="lps-button" onclick="return confirm(\'Are you sure you want to delete this log?\');">Delete</a>
            <a href="' . esc_url( admin_url( 'admin-post.php?action=lps_end_session&user_id=' . esc_attr( $log->user_id ) ) ) . '" class="lps-button">End Session</a>
        </td>';
		}
...
// Add hooks to handle form submissions
add_action( 'admin_post_lps_delete_all_login_logs', 'lps_handle_delete_all_logs' );
add_action( 'admin_post_lps_delete_all_failed_logs', 'lps_handle_delete_all_logs' );
...
// Handle delete all failed login logs
function lps_handle_delete_all_failed_logs() {
	global $wpdb;

	if ( isset( $_POST['delete_all_failed_logs'] ) && current_user_can( 'manage_options' ) ) {
		// Verify nonce
		if ( ! isset( $_POST['lps_nonce_field'] ) || ! wp_verify_nonce( $_POST['lps_nonce_field'], 'lps_delete_all_logs_nonce' ) ) {
			wp_die( 'Nonce verification failed' );
		}
		...
		$wpdb->query( "DELETE FROM $table_name" ); // Delete all failed login logs
