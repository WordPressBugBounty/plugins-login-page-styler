jQuery( document ).ready(
	function ($) {
		...
		function initializeBgSizeSliders() {
			$( '#lps_login_logo_width, #lps_login_logo_height' ).on(
				'input',
				function () {
		...
		var lpsTemplate = lpsData.layout;

		switch (lpsTemplate) {
			case 'lay1':
				initializeColorPicker( 'lps_login_form_color', '.login form, .login h1', 'background' );
				break;
			case 'lay2':
				initializeColorPicker( 'lps_login_form_color', '.login form, .login h1', 'background' );
				break;
		...
		initializeColorPicker( 'lps_login_nav_bgcolor', '#backtoblog a, #nav a', 'background' );
		initializeColorPicker( 'lps_login_nav_bgcolor_hover', '#backtoblog a, #nav a', 'background',true );
		...
		function initializeRangeSlider(inputId) {
			...
			$( '#amountInput' + inputId.charAt( inputId.length - 1 ) ).on(
				'input',
				function () {
					var value = $( this ).val();
		...
		function applyGoogleFontToIframe(fontFamily, $iframeContent, targetSelector) {
			...
			$iframeContent.find( targetSelector ).attr(
				'style',
				function (i, style) {
		...
		function applyAnimationToIframe(targetSelector, animationName, duration) {
			...
			setTimeout(
				...
				10
			); // Slight delay to apply the new animation
		}
		...
		document.getElementById( 'lps_login_form_position' ).addEventListener(
			'change',
			function () {
		...
		function applyBoxShadowToTarget(boxShadowValue) {
			...
			$iframeContent.find( '.login form' ).each(
				function () {
		...
		function initializeBoxShadowInputs() {
			...
			$( '#lps_box_shadow_horizontal, #lps_box_shadow_vertical, #lps_box_shadow_blur, #lps_box_shadow_spread' ).on(
				'input.boxshadow',
				function () {
			...
			$( '#amountInputBSH, #amountInputBSV, #amountInputBSBlur, #amountInputBSSpread' ).on(
				'input.boxshadow',
				function () {
					var value = $( this ).val();
